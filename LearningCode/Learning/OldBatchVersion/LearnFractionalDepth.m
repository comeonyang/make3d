...
% *
% */
function LearnFracationalDepth(LearnAlg,AbsFeaType,AbsFeaDate,WeiBatchNumber,logScale,SkyExclude,LearnNear)
...
%    [thi{l},stats] = robustfit(FeaVector',target{l},'huber');
    ...
    pack;
    whos;
    MaxEig = max(svd(FeaVector));
    save([ ScratchDataFolder '/data/MaxEigFraDepthL.mat' ],'MaxEig','RegularWei','FeaVector');
return;
    if strcmp(LearnAlg,'L1norm')
        tic
        opt = sdpsettings('solver','sedumi');
        thit = sdpvar(Fsize,1);
        solvesdp([],norm( ([ones(Tsize,1) FeaVector']./repmat(target{l},[1 Fsize]))*thit - ones(Tsize,1) , 1), opt);
        thi{l} = thit;
        % base line learning 
        X_base = ones( Tsize,1);
	tic
        cvx_begin
           cvx_quiet(true);
           variable thit(1);
           minimize (norm( (X_base./target{l})*thit - ones(Tsize,1),1));
        cvx_end
        thi_base{l} = thit;
        toc
        % error measure
        error{l} = abs( ([ones(Tsize,1) FeaVector']./repmat(target{l},[1 Fsize]))*thi{l} - ones(Tsize,1));
        error_base{l} = abs( (X_base./target{l})*thi_base{l} - ones(Tsize,1));
        learnRatio{l} = sum(error{l})/sum(error_base{l});
    elseif strcmp(LearnAlg,'robustfit')
       if GassuianRegularization
        RegM = diag(ones(Fsize,1));
        size(RegM)
        [thi{l},stats] = robustfit([ ([ones(Tsize,1) FeaVector']./repmat(target{l},[1 Fsize])) ;RegM*RegularWei*MaxEig], [ones(Tsize,1); zeros(Fsize,1) ], 'huber', 1.345,'off');
       else
        [thi{l},stats] = robustfit(([ones(Tsize,1) FeaVector']./repmat(target{l},[1 Fsize])), ones(Tsize,1), 'huber', 1.345,'off');
       end
        % base line learning
        X_base = ones(Tsize,1);
        [thi_temp,stats] = robustfit((X_base./target{l}), ones(Tsize,1), 'huber', 1.345,'off');
    ...
    elseif strcmp(LearnAlg,'L2norm')
        thi{l} = ([ones(Tsize,1) FeaVector']./repmat(target{l},[1 Fsize]))\ones(Tsize,1); 
        % base line learning
        X_base = ones( Tsize,1);
        thi_base{l} = (X_base./target{l})\ ones(Tsize,1);
        % error measure
        error{l} = abs( ([ones(Tsize,1) FeaVector']./repmat(target{l},[1 Fsize]))*thi{l} - ones(Tsize,1));
        error_base{l} = abs( (X_base./target{l})*thi_base{l} - ones(Tsize,1));
        learnRatio{l} = sum(error{l})/sum(error_base{l});
...
DateStamp = date;
if GassuianRegularization
  save([ScratchDataFolder '/../learned_parameter/FractionalRegDepth_' ImgFolder '_' LearnAlg ...
  ...
  save([ScratchDataFolder '/../learned_parameter/FractionalDepth_' ImgFolder '_' LearnAlg ...
      '_Nonsky' num2str(SkyExclude) '_Log' num2str(logScale) ...
      '_Near' num2str(LearnNear) '_WeiBatNu' num2str(WeiBatchNumber) ...
      '_' AbsFeaType '_AbsFeaDate' AbsFeaDate  '_LearnDate' DateStamp '.mat'],...
      'thi','thi_base','error','error_base','learnRatio');
