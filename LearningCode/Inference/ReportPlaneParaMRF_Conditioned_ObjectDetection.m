...
% *
% */
function  ReportPlaneParaMRF_Conditioned_ObjectDetection(Default, step, DepthFolder,...
          Sup, SupO, MedSup,depthMap,VarMap,RayOri, Ray, SupNeighborTable,...
          MedRay,maskSky,maskG, CornerList, OccluList,...
...
% =============== Process the SupO and SupB =========Min Aug 13th
SE = strel('diamond', 1);
ExpandZeros = imdilate( Sup ==0, SE);
SupOri = Sup;
SupO( ~logical( ExpandZeros ) ) = 0;
SupO( SupO > 0) = 0;

SupTemp = Sup;
SupTemp( ExpandZeros & SupTemp <=0) = 0;
SupB = FillIndex(SupTemp);
SupB( ~logical( ExpandZeros ) ) = 0;
%SupB( SupB < 0) = 0;
...
% clean the depth if Sup <0 //Min added Aug 10th
CleanedDepthMap = depthMap;
CleanedDepthMap(Sup <0 ) = NaN;
...
%   NuPatch = Default.HoriXNuDepth*Default.VertYNuDepth-sum(maskSky(:));
   ...
   MinSupInd = min(Sup(:)) - 1;% might be negative
   Sup2Para = sparse(1,max(Sup(:)) - MinSupInd);
   Sup2Para(NuSup - MinSupInd) = 1:NuSupSize;

   % Min add for SupInd can be negative ==============
	ShiftedSup = Sup-MinSupInd;% Min add for proper used for Matrix indexing
	ShiftSupEpand = SupEpand - MinSupInd;
   % =================================================
...
beta = [];
EmptyIndex = [];
for i = (NuSup - MinSupInd)
%    mask = Sup ==i;
    mask = ShiftSupEpand ==i; % include the Ray that will be use to expand the NonSky
    RayAllOriM = blkdiag( RayAllOriM, RayOri(:,mask)');
    RayAllM = blkdiag( RayAllM, Ray(:,mask)');
    mask = ShiftedSup ==i; % Not include the Ray that will be use to expand the NonSky    
    [yt x] = find(mask);
    ...
    SupNuPatch(i) = sum(mask(:)); % Min modify i to be positive since SupNuPatch(i) is used
...
for i = NuSup(NuSup > 0) % CoPlane place only on Positive index Sup
    ...
    for j = Neighbor
        ...
        temp1(:,(Sup2Para( i - MinSupInd)*3-2): Sup2Para( i - MinSupInd)*3) = ...
                                                   RayOri(:,CenterY,CenterX)';
        temp2(:,(Sup2Para( j - MinSupInd)*3-2): Sup2Para( j - MinSupInd)*3) = ...
                                                   RayOri(:,CenterY,CenterX)';
    %NuNei-NuSTList
...
%       wei = WeiV*10;%*(MultiScaleSupTable(Sup2Para(ClosestNList(i,1)),2:end) == MultiScaleSupTable(Sup2Para(ClosestNList(i,2)),2:end))';  
           if MultiScaleFlag 
              vector = (MultiScaleSupTable(Sup2Para( i - MinSupInd),2:end) == MultiScaleSupTable(Sup2Para( j - MinSupInd),2:end));
...
disp(' check Negative SupIND have no CoP terms');
sum(sum( CoPM1(:,1:6)))
sum(sum( CoPM2(:,1:6)))
...
SupPixelNeighborList = sparse( max(ShiftedSup(:)), max(ShiftedSup(:)) );
SupPixelParsedList = sparse( max(ShiftedSup(:)), max( ShiftedSup(:)) );
recordAdded1 = sparse( max( ShiftedSup(:)), max( ShiftedSup(:)) );
recordAdded2 = sparse( max( ShiftedSup(:)), max( ShiftedSup(:)) );
addedIndexList = [ ];

BounaryPHori = conv2(ShiftedSup,[1 -1],'same') ~=0;
BounaryPHori(:,end) = 0;
BounaryPVert = conv2(ShiftedSup,[1; -1],'same') ~=0;
BounaryPVert(end,:) = 0;

%    boundariesHoriIndex = find(BounaryPHori==1)';
% pre-select the boundary in order with the NuSup order
for l = NuSup( NuSup > 0) %  Only stich Positive Sup index in Horizontal direction
    mask = Sup == l;
    boundariesHoriIndex = find(BounaryPHori==1 & mask)';
    %boundariesHoriIndex = boundariesHoriIndex( Sup(boundariesHoriIndex) > 0); % Only stich Positive Sup index in Horizontal direction
    for i = boundariesHoriIndex
        j = i+Default.VertYNuDepth;
        if Sup(i) <= 0 || Sup(j) <= 0 % Min changed to <=0 since Negative Sup is possible
           continue;
        end
        SupPixelParsedList(ShiftedSup(i),ShiftedSup(j)) = SupPixelParsedList(ShiftedSup(i),ShiftedSup(j)) + 1;
    
        if SupPixelNeighborList(ShiftedSup(i),ShiftedSup(j)) == 0
           recordAdded1(ShiftedSup(i),ShiftedSup(j)) = i;     
        elseif SupPixelNeighborList(ShiftedSup(i),ShiftedSup(j)) >= MAX_POINTS_STITCH_HORI
           continue;
        elseif SupPixelNeighborList(ShiftedSup(i),ShiftedSup(j)) == 1  % inside this remove the close stiching terms
           rowN = ceil(i/55);      colN = rem(i,55);
           rowN_older = ceil( recordAdded1(ShiftedSup(i),ShiftedSup(j)) / 55);
           colN_older = rem( recordAdded1(ShiftedSup(i),ShiftedSup(j)), 55);
           if abs(rowN - rowN_older) + (55/305)*abs(colN - colN_older) > DIST_STICHING_THRESHOLD_HORI && ...
              ...
              recordAdded2(ShiftedSup(i),ShiftedSup(j)) = i;
           else
              continue;
           end
        elseif SupPixelNeighborList(ShiftedSup(i),ShiftedSup(j)) == 2     %Assuming MAX_POINTS_STITCH = 3
           rowN = ceil(i/55);      colN = rem(i,55);
           rowN_older1 = ceil( recordAdded1(ShiftedSup(i),ShiftedSup(j)) / 55);
           colN_older1 = rem( recordAdded1(ShiftedSup(i),ShiftedSup(j)), 55);
           rowN_older2 = ceil( recordAdded2(ShiftedSup(i),ShiftedSup(j)) / 55);
           colN_older2 = rem( recordAdded2(ShiftedSup(i),ShiftedSup(j)), 55);
        ...
        SupPixelNeighborList(ShiftedSup(i),ShiftedSup(j)) = SupPixelNeighborList(ShiftedSup(i),ShiftedSup(j)) + 1;
...
for i = addedIndexList
    ...
    WeightHoriNeighborStitch = [WeightHoriNeighborStitch;  SupPixelParsedList(ShiftedSup(i),ShiftedSup(j)) / ...
                                            SupPixelNeighborList(ShiftedSup(i),ShiftedSup(j)) ];

    Target(1) = Sup2Para(Sup(i) - MinSupInd);
    Target(2) = Sup2Para(Sup(j) - MinSupInd);
...
%    betaTemp = StickHori*WeiV;%*(MultiScaleSupTable(Sup2Para(Sup(i)),2:end) == MultiScaleSupTable(Sup2Para(Sup(j)),2:end))';%*(DistStickLengthNormWei.^2)*beta(Target(I));
    if MultiScaleFlag
          vector = (MultiScaleSupTable(Sup2Para(Sup(i) - MinSupInd),2:end) == MultiScaleSupTable(Sup2Para(Sup(j) - MinSupInd),2:end));
...
%    for i = find(BounaryPVert==1)'
    for i = find(BounaryPVert==1 & mask)'
        j = i+1;
        if Sup(i) == 0 || Sup(j) <= 0 % Min changed since the bottom Sup_j can't be negative index
           continue;
        end
        SupPixelParsedList(ShiftedSup(i),ShiftedSup(j)) = SupPixelParsedList(ShiftedSup(i),ShiftedSup(j)) + 1;
        ...
        if SupPixelNeighborList(ShiftedSup(i),ShiftedSup(j)) == 0
           recordAdded1(ShiftedSup(i),ShiftedSup(j)) = i;
        elseif SupPixelNeighborList(ShiftedSup(i),ShiftedSup(j)) >= MAX_POINTS_STITCH_VERT
           continue;
        elseif SupPixelNeighborList(ShiftedSup(i),ShiftedSup(j)) == 1  % inside this remove the close stiching terms
           rowN = ceil(i/55);      colN = rem(i,55);
           rowN_older = ceil( recordAdded1(ShiftedSup(i),ShiftedSup(j)) / 55);
...
for i = addedIndexList
    j = i+1;
     ...
     WeightVertNeighborStitch = [WeightVertNeighborStitch;  SupPixelParsedList(ShiftedSup(i),ShiftedSup(j)) / ...
                                            SupPixelNeighborList(ShiftedSup(i),ShiftedSup(j)) ];
...
for k = 0:(TotalRectX-1)
  ...
  for q = 0:(TotalRectY-1)
    ...
    PositiveSubSup = SubSup( SubSup>0)
    BoundarySup = find(sum(SupNeighborTable( PositiveSubSup,:), 1) ~=0);
    BoundarySup = unique(setdiff(BoundarySup,[0 PositiveSubSup] ));
    
	% chech if BoundarySup non-NaN in PlanePara
    checkNoNNaN = ~isnan(PlanePara(Sup2Para(BoundarySup - MinSupInd)*3));
    BoundarySup = BoundarySup(checkNoNNaN);
    TotalSup = sort([SubSup BoundarySup]);

    SubSupPtr = [ Sup2Para(SubSup - MinSupInd)*3-2;...
                    Sup2Para(SubSup - MinSupInd)*3-1;...
                    Sup2Para(SubSup- MinSupInd)*3];
    SubSupPtr = SubSupPtr(:);
    BoundarySupPtr = [ Sup2Para(BoundarySup - MinSupInd)*3-2;...
                       Sup2Para(BoundarySup - MinSupInd)*3-1;...
                       Sup2Para(BoundarySup - MinSupInd)*3];
    ...
    for i = step
        ...
        F = set(ParaPPCP(3*(1:NuSubSupSize)-1).*YPointer(Sup2Para(SubSup - MinSupInd))<=0)...
...
%mean(abs(ParaPPCP(:)))  
   ...
   fprintf(['     ' num2str( toc(inferenceTime) ) '\n     : In 1st level Optimization, using new solver.' ...
	...
	temp(3*(1:NuSubSupSize)-1) = YPointer(Sup2Para(SubSup - MinSupInd));
...
% build the whole image
    ...
    % porject the ray on planes to generate the ProjDepth
    FitDepthPPCP = FarestDist*ones(1,Default.VertYNuDepth*Default.HoriXNuDepth);
    FitDepthPPCP(~maskSkyEroded) = (1./sum(PlanePara(:,Sup2Para(ShiftSupEpand(~maskSkyEroded ) )).*Ray(:,~maskSkyEroded ),1))';
    FitDepthPPCP = reshape(FitDepthPPCP,Default.VertYNuDepth,[]);
    % storage for comparison purpose ======================
%    depthMap = FarestDist*ones(1,Default.VertYNuDepth*Default.HoriXNuDepth);
...
%  verticalThreshold = cos(50*pi/180);
  ...
  for i = NuSup
      mask = Sup == i;
      if any(verticalParaInd == Sup2Para(i - MinSupInd))
...
% Start Decompose image =========================
  ...
  for k = 0:(TotalRectX-1)
	...
	PositiveSubSup = SubSup( SubSup > 0);
      ...
      BoundarySupPtr =BoundarySupPtr(:);
      NuSubSupSize = size(SubSup,2);
      SubMinSupInd = min(SubSup(:)) -1;%might be negative
      SubSup2Para = sparse(1,max(SubSup) - SubMinSupInd);
      SubSup2Para(SubSup - SubMinSupInd) = 1:NuSubSupSize;
...
%    try reduce the vertical constrain
     NonVertPtr = setdiff( 1:(3*NuSubSupSize), SubSup2Para( NuSup( (intersect( indexVertical,SubSupPtr ) +1)/3) - SubMinSupInd)*3-1 );
     YNoComingBack = YPointer(Sup2Para(SubSup - MinSupInd));
     YNoComingBack(SubSup2Para( NuSup( (intersect( indexVertical,SubSupPtr ) +1)/3) - SubMinSupInd)) = [];
     ...
     XCompMask( SubSup2Para( NuSup( (intersect( indexGroundX,SubSupPtr ) +2)/3) - SubMinSupInd)*3-2 ) = 1;
     XCompMask = XCompMask(NonVertPtr);
     ZCompMask = zeros(1,3*NuSubSupSize);
     ZCompMask( SubSup2Para( NuSup( (intersect( indexGroundZ,SubSupPtr ) )/3) - SubMinSupInd)*3 ) = 1;
...
%  	fprintf([ '    Finished new solver at: ' num2str( toc(inferenceTime) ) '\n ' ]);
  ...
  FitDepth(~maskSkyEroded) = (1./sum(PlanePara(:,Sup2Para(SupEpand(~maskSkyEroded) - MinSupInd)).*Ray(:,~maskSkyEroded),1))';
  FitDepth = reshape(FitDepth,Default.VertYNuDepth,[]);
  % ================  add boundary of object and back ground
	% EXTEND OBJECT
  FitDepthObject = FarestDist*ones(1,Default.VertYNuDepth*Default.HoriXNuDepth);
  FitDepthObject( SupO < 0) = (1./sum(PlanePara(:,Sup2Para(SupO( SupO <0) - MinSupInd)).*Ray(:, SupO < 0),1))';
  FitDepthObject = reshape(FitDepthObject,Default.VertYNuDepth,[]);
	% EXTEND Back Ground
  FitDepthBackGround = FarestDist*ones(1,Default.VertYNuDepth*Default.HoriXNuDepth);
  FitDepthBackGround( SupB > 0) = (1./sum(PlanePara(:,Sup2Para(SupB( SupB > 0) - MinSupInd)).*Ray(:, SupB > 0),1))';
  FitDepthBackGround = reshape(FitDepthBackGround,Default.VertYNuDepth,[]);
  % ========================================================
  ...
  % ================  add boundary of object and back ground
	% EXTEND OBJECT
  [Position3DFitedObject] = im_cr2w_cr(FitDepthObject,permute(Ray,[2 3 1]));
  Position3DFitedObject(3,:) = -Position3DFitedObject(3,:);
  Position3DFitedObject = permute(Position3DFitedObject,[2 3 1]);
	% EXTEND Back Ground
  [Position3DFitedBackGround] = im_cr2w_cr(FitDepthBackGround,permute(Ray,[2 3 1]));
  Position3DFitedBackGround(3,:) = -Position3DFitedBackGround(3,:);
  Position3DFitedBackGround = permute(Position3DFitedBackGround,[2 3 1]);
  % ========================================================
  RR =permute(Ray,[2 3 1]);
  temp = RR(:,:,1:2)./repmat(RR(:,:,3),[1 1 2]);
  ...
  PositionTex = permute(PositionTex,[2 3 1]);
	...
	if true % Min try true testing only Aug 3rd%Min add false since WrlFacestHroiReduce is broken right now
  fprintf(['     ' num2str( toc(inferenceTime) ) '\n     : Writing WRL.']);
 ...
 % add boundary of object and back ground =====================
	% EXTEND OBJECT
 WrlFacestHroiReduce( Position3DFitedObject,PositionTex,SupO, Default.filename{1}, Default.filename{1}, ...        
		     Default.OutPutFolder, GridFlag, 1);
	% EXTEND Back Ground
 WrlFacestHroiReduce( Position3DFitedBackGround,PositionTex,SupB, Default.filename{1}, Default.filename{1}, ...        
		     Default.OutPutFolder, GridFlag, 1);
 % ============================================================
