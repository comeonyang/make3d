...
% *
% */
function  [ ModelInfo] = PlaneParaMRFTriangulateOneShot...
          ...
          Scale, CoordinateFromRef, y0);
%function  [Position3DFited FitDepth] = PlaneParaMRFTriangulateOneShot...
...
OcclusionThre = 0.5;% 5 Meters or bigger jump means occlusion
scale = 1;
StickHori = 5;  %0.1; % sticking power in horizontal direction
StickVert = 10; % sticking power in vertical direction % 5 cause wall seperating ground
Center = 5; %10 Co-Planar weight at the Center of each superpixel
TriangulatedWeight = 200;
phi = 1;
%shift = (2+Default.TriCountSupThre)/2 % half-wei  is at the shift point (used 0.5 ) 1 still too big (2 )
shift = (Default.TriCountSupThre*1.5)/2; % half-wei  is at the shift point (used 0.5 ) 1 still too big (2 )
LogisticPara = [[1 1]; [0 1]]\[TriangulatedWeight; 1];
...
FarestDist = 250;%1.5*max(ClosestDepth)% Min Modified July 1st250%1.5*median(depthMap(:)); % tried on university % nogood effect but keep it since usually it the rangelike this   % change to 1.5 for church
% ================================================
...
% ============ last mintune change by Min , not to render far away point
%SupOri(depthMap>FarestDist) = 0; % Index 0 mean Sky
%Sup(depthMap>FarestDist) = 0; % Index 0 mean Sky
% ===========
...
%verticalThreshold = cos([ 15*ones(1,Default.VertYNuDepth - ceil(Default.VertYNuDepth/2)) ...
%				linspace(15,55,ceil(Default.VertYNuDepth/2))]*pi/180); 
...
for i = NuSup
    ...
    % make sure Neighbor contain no sky since Sup modified to zero give
    % depths too far away
    Neighbor = intersect( Neighbor,NuSup);
    for j = Neighbor
        %  if ~CornerList(i)
        mask = Sup == i;
...
%        end
	...
	%//Min add for triangulation related CoPM wei
	CenterX_i = CenterX;    
	CenterY_i = CenterY;    
        ...
        temp2(:,(Sup2Para( j)*3-2): Sup2Para( j)*3) = ...
                                                            Ray(:,CenterY,CenterX)';
	% //Min add for triangulation related CoPM wei
	Img_i_posi = Ray(:,CenterY_i,CenterX_i);
	Img_i_posi = Img_i_posi(1:2)./Img_i_posi(3);
	Img_j_posi = Ray(:,CenterY,CenterX);
	Img_j_posi = Img_j_posi(1:2)./Img_j_posi(3);
	Img_posi = (Img_i_posi + Img_j_posi)/2;
	ClosetTriImgPosi = FindClosetTriImgPosi( Img_posi, RayMatched);
	Dist2ClosetTri = sqrt(sum((ClosetTriImgPosi - Img_posi).^2));
	if Dist2ClosetTri >= Default.FarestTriDist
		ModifiedWei = 1;
	else
		% reduce weight when close to Triangulated point
		%ModifiedWei = Default.MinTriEffectPercent + (1 - Default.MinTriEffectPercent)*(Dist2ClosetTri/Default.FarestTriDist); 
		% increase weight when close to Triangulated point
                ModifiedWei = 1 + (Default.MinTriEffectPercent)*(1 - (Dist2ClosetTri/Default.FarestTriDist)); 
	end
    ...
    % (do not divide by 2 because the penalty should be double.
       ...
       CoPM1 = [CoPM1; (temp1*wei + oneRay1)*ModifiedWei];
       CoPM2 = [CoPM2; (temp2*wei + oneRay2)*ModifiedWei];
...
%    for m = SubSup
    ...
    NewScalingTerm = ScalingTerm(tar);
	...
	NewWeightVertNeighborStitch = WeightVertNeighborStitch(tar);
        ...
        ParaTar = NewPosiM(:, 3:3:(3*NuSubSupSize)) + NewPosiM(:, 2:3:(3*NuSubSupSize)) +NewPosiM(:, 1:3:(3*NuSubSupSize));
        [row col] = find(ParaTar);
        TriParaTar = NewPosiTriangulatedM(:, 3:3:(3*NuSubSupSize)) + NewPosiTriangulatedM(:, 2:3:(3*NuSubSupSize)) +NewPosiTriangulatedM(:, 1:3:(3*NuSubSupSize));
        [TriRow TriCol] = find(TriParaTar);
	TriCountSup = histc(TriCol,1:NuSubSupSize); % Min add to count the Tri per sup
	TriColConfident = find( TriCountSup > Default.TriCountSupThre);
    
    TriLogisticWei = (1./(1+exp( - phi.*( TriCountSup(TriCol) - shift)))) *LogisticPara(1)+LogisticPara(2);
    if isempty(TriLogisticWei)
        TriLogisticWei = zeros(0,1);
    end    
...
%           sol = solvesdp(F, norm( (NewPosiM*ParaPPCP-( ScaleP(col).*NewScalingTerm +ones(size(NewPosiM,1) ,1)) ) .* WeightsSelfTerm,1)...
          %sol = solvesdp(F, norm( (NewPosiM*ParaPPCP-ScaleP(col) ) .* WeightsSelfTerm,1)...    
 ...
 %                   + SampledGroundWeight*norm( NewPosiSampledGroundM*ParaPPCP-ones(size(NewPosiSampledGroundM,1), 1), 1)+...
    ...
    ScaleP = double( ScaleP);
    ParaPPCP = reshape( ParaPPCP, 3, [])./ repmat(ScaleP', 3, 1);
    ParaPPCP = ParaPPCP(:);
    sum(isnan(ParaPPCP))/prod(size(ParaPPCP))
%     pause
    yalmip('clear');
...
% build the whole image
    ...
    NaNMark = isnan(FitDepthPPCP);
	FitDepthPPCP(NaNMark) = CleanedDepthMap(NaNMark);
	FitDepthPPCP(isnan( FitDepthPPCP)) = FarestDist;
    FitDepthPPCP = reshape(FitDepthPPCP,Default.VertYNuDepth,[]);
	sum(isnan(FitDepthPPCP(:)))
%     pause
    % =====================================================
...
%         Position3DFitedPPCP = Rotation*Position3DFitedPPCP(:,:) + repmat(Translation, 1, length(Position3DFitedPPCP(:,:)));
         ...
         if appendOpt == 1
%		[VrmlName] = new_vrml_test_faceset_goodSkyBoundary( Default.ScratchFolder, ['../' Default.filename{1}], permute( Position3DFitedPPCP,[3 1 2]), [], permute(Ray,[2 3 1]), [ Default.Wrlname{1} '1st'], ...
%                    [], [], 0, logical(zeros(55,305)), logical(zeros(55,305)), 1, 0, Default.a_default, Default.b_default, Default.Ox_default, Default.Oy_default, 1);
             WrlFacestHroiReduce(Position3DFitedPPCP,PositionTex,SupOri, [ Default.filename{1} ],[ Default.Wrlname{1} '1st'], ...
                             Default.OutPutFolder, GridFlag, 1);
         else
%		[VrmlName] = new_vrml_test_faceset_goodSkyBoundary( Default.ScratchFolder, ['../' Default.filename{1}], permute( Position3DFitedPPCP,[3 1 2]), [], permute(Ray,[2 3 1]), [ Default.Wrlname{1} '1st'], ...
%                    [], [], 0, logical(zeros(55,305)), logical(zeros(55,305)), 1, 0, Default.a_default, Default.b_default, Default.Ox_default, Default.Oy_default, 0);
             WrlFacestHroiReduce(Position3DFitedPPCP,PositionTex,SupOri, [ Default.filename{1} ],[ Default.Wrlname{1} '1st'], ...
...
if NoSecondStep
    Position3DFited = Position3DFitedPPCP;
    FitDepth = FitDepthPPCP;
	return;
end
...
%  verticalThreshold = cos(50*pi/180);
  ...
  % Check Y component of the Surface Norm (Normalized PlaneParameter )
...
%  verticalPara = abs(normalizedPara(2,:)) <= verticalThreshold(YPosition); % ssuming ground has [0; 1; 0] surface norm
  ...
  for i = NuSup
      mask = Sup == i;
     if any(verticalParaInd == Sup2Para(i)) %still keep the vertical para spatial support
...
% %         size(PosiMPPCP)
    ...
    if ~isempty(y0)    
      disp('y0 constrain');
      %NewAlphaGround = Rotation'*[0; 1/y0; 0]*Scale/(1-Translation'*[0; 1/y0; 0]); % the same as Rotation(2,1)'./(y0 - Translation(2))
      F = set( CoordinateFromRef(:,2)'*reshape(Para,3,[]).*YNoComingBack'<=0)...
          ...
          +set( Para( XCompMask*3-2) == Rotation(2,1)*Scale./(y0 - Translation(2)) )...
          +set( Para( XCompMask*3-1) == Rotation(2,2)*Scale./(y0 - Translation(2)) )...
          +set( Para( XCompMask*3) == Rotation(2,3)*Scale./(y0 - Translation(2)) );
          ...
          %;
       size(NewPosiMPPCP*Para-ones(size(NewPosiMPPCP,1),1))
       size( abs(NewVarM)/BandWith)
       size( ( CoordinateFromRef(:,1)'*reshape( Para( reshape( [XCompMask*3-2; XCompMask*3-1 ;XCompMask*3], [], 1) ) , 3, []) ))
       size( normPara( GroundMask ))
       size( ( CoordinateFromRef(:,2)'*reshape( Para( reshape( [YCompMask*3-2; YCompMask*3-1 ;YCompMask*3], [], 1) ) , 3, [])))
       size( normPara( VertMask ))
       tempG1 = ( CoordinateFromRef(:,1)'*reshape( Para( reshape( [XCompMask*3-2; XCompMask*3-1 ;XCompMask*3], [], 1) ) , 3, []) );
       tempG2 = ( CoordinateFromRef(:,3)'*reshape( Para( reshape( [XCompMask*3-2; XCompMask*3-1 ;XCompMask*3], [], 1) ) , 3, []) );
       if size(tempG1,2) == 0
          tempG1 = zeros(0,0);
          tempG2 = zeros(0,0);
       end
       tempV = ( CoordinateFromRef(:,2)'*reshape( Para( reshape( [YCompMask*3-2; YCompMask*3-1 ;YCompMask*3], [], 1) ) , 3, []));
       if size(tempV,2) == 0
          tempV = zeros(0,0);
       end
        TriParaTar = NewPosiTriangulatedM(:, 3:3:(3*NuSubSupSize)) + NewPosiTriangulatedM(:, 2:3:(3*NuSubSupSize)) +NewPosiTriangulatedM(:, 1:3:(3*NuSubSupSize));
...
%        Objective = [];
        if isempty(tempG1)
%            Objective = GroundWeight*norm( tempG1./...
%                              normPara( GroundMask ), 1)...
%                   +GroundWeight*norm( tempG2./...
%                              normPara( GroundMask ), 1);
            GroundMask = logical(zeros(size( tempG1)));
        end
        if isempty(tempV)
%            Objective = Objective +VertWeight*norm( tempV./...
%                              normPara( VertMask ), 1);
            VertMask = logical(zeros(size( tempV)));
        end
%        if isempty(Objective)
%       sol=solvesdp(F,norm( ( NewPosiMPPCP*Para-ones(size(NewPosiMPPCP,1),1)),1)...
...
%        else
        sol=solvesdp(F,norm( ( NewPosiMPPCP*Para-ones(size(NewPosiMPPCP,1),1)),1)...
                   +norm( (NewPosiTriangulatedM*Para-ones(size(NewPosiTriangulatedM,1), 1)).*TriLogisticWei, 1)+...
                   +SampledGroundWeight*norm( NewPosiSampledGroundM*Para-ones(size(NewPosiSampledGroundM,1), 1), 1)+...
                   +Center*norm(( NewCoPM*Para + NewCoPMBound).*NewCoPEstDepth, 1)...
                   +norm(( NewHoriStickM*Para + NewHoriStickMBound).*...
                           NewEstDepHoriStick.*NewWeightHoriNeighborStitch,1)...
                   +norm(( NewVertStickM*Para + NewVertStickMBound).*...
                           NewEstDepVertStick.*NewWeightVertNeighborStitch,1)...
                   +GroundWeight*norm( tempG1./...
                              normPara( GroundMask ), 1)...
                   +GroundWeight*norm( tempG2./...
                              normPara( GroundMask ), 1) ...
                   +VertWeight*norm( tempV./...
                              normPara( VertMask ), 1) ...
                   , opt);
                    % test tri satisfied or not
                    disp('resudual of Tri point')
                    norm( NewPosiTriangulatedM*double(Para)-ones(size(NewPosiTriangulatedM,1), 1), 1)
%        end
%                   +GroundWeight*norm( ( CoordinateFromRef(:,1)'*reshape( Para( reshape( [XCompMask*3-2; XCompMask*3-1 ;XCompMask*3], [], 1) ) , 3, []) )./...
%                              normPara( GroundMask ), 1)...
%                   +GroundWeight*norm( ( CoordinateFromRef(:,3)'*reshape( Para( reshape( [XCompMask*3-2; XCompMask*3-1 ;XCompMask*3], [], 1) ) , 3, []))./...
%                              normPara( GroundMask ), 1) ...
%                   +VertWeight*norm( ( CoordinateFromRef(:,2)'*reshape( Para( reshape( [YCompMask*3-2; YCompMask*3-1 ;YCompMask*3], [], 1) ) , 3, []))./...
%                              normPara( VertMask ), 1) ...
...
%     PlanePara(TotalSupPtr) = Para;
  ...
  % ---Min add July 1st
  FitDepth(isnan(FitDepth)) = FarestDist;
  % -------------------

  % ===============Occlusion detection ======================
if false
  DiffHori = conv2(FitDepth, [1 -1], 'valid');
  DiffVert = conv2(FitDepth, [1; -1], 'valid');
  maskH = [ (DiffHori./FitDepth(:,2:end) > OcclusionThre) zeros(size(FitDepth,1),1)];
  maskH(end,:) = 0;
  maskV = [ (DiffVert./FitDepth(2:end,:) > OcclusionThre); zeros(1, size(FitDepth,2))];
  maskV(:,end) = 0;
  SupOri(logical(  maskH) ) = 0;
  SupOri(logical(  maskV) ) = 0;
end
  % =========================================================
...
%  Position3DFited = Rotation*Position3DFited(:,:) + repmat(Translation, 1, length(Position3DFited(:,:)));
  ...
  % =============== Building up ModelInfo==================
  ModelInfo.PlanePara = PlanePara;
  ModelInfo.Sup2Para = Sup2Para;
  ModelInfo.SupEpand = SupEpand;
  ModelInfo.maskSkyEroded = maskSkyEroded;
  ModelInfo.Ray = Ray;
  ModelInfo.PositionTex = PositionTex;
  ModelInfo.FitDepth = FitDepth;
  ModelInfo.Position3DFited = Position3DFited;
  ModelInfo.SupOri = SupOri;
  % =======================================================

 if Default.RenderFlag
  fprintf(['     ' num2str( toc(inferenceTime) ) '\n     : Writing WRL.']);
  if appendOpt == 1
%      [VrmlName] = new_vrml_test_faceset_goodSkyBoundary( Default.ScratchFolder, ['../' Default.filename{1}], permute( Position3DFited,[3 1 2]), [], permute(Ray,[2 3 1]), [ Default.Wrlname{1} ], ...
%                    [], [], 0, logical(zeros(55,305)), logical(zeros(55,305)), 1, 0, Default.a_default, Default.b_default, Default.Ox_default, Default.Oy_default, 1);
