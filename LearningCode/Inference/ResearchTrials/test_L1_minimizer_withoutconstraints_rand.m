...
% *
% */
function [t]= test_L1_minimizer_withoutconstraints(filename,m,n)
...
%   OutPutFolder = 'C:\Documents and Settings\Ash\My Documents\Reconstruction3d\DataTemp\';
   load([ScratchFolder 'Catsh1stOpt_' filename '.mat']);
...
%inq = q;
tic;
[x_ashIterator, alfa, status, history, T_nt_hist] = SigmoidSolver( Para, [], [], [], '', [], [], 0);
...
alfa
tic
[x_ashIterator, status, history] = L1Barrier_wo_Constrain(Para,[],[],[],0);
toc
t(2,1) = toc;
%return;
% generate VRml
...
sol = solvesdp([], norm(A*xsedumi-b, 1),opt);
