...
OcclusionThre = 1;% 5 Meters or bigger jump means occlusion
scale = 1;
...
Center = 10; %10 Co-Planar weight at the Center of each superpixel
...
FarestDist = 250%1.5*median(depthMap(:)); % tried on university % nogood effect but keep it since usually it the rangelike this   % change to 1.5 for church
% ================================================
...
if false
  ...
  maskH = [ (DiffHori > OcclusionThre) zeros(size(FitDepth,1),1)];
  maskH(end,:) = 0;
  maskV = [ (DiffVert > OcclusionThre); zeros(1, size(FitDepth,2))];
  maskV(:,end) = 0;
