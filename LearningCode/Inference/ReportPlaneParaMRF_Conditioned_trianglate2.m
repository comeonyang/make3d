...
% *
% */
function  [Position3DFited FitDepth] = ReportPlaneParaMRF_Conditioned_trianglate2...
...
Center = 2.5; % Co-Planar weight at the Center of each superpixel
TriangulatedWeight = 100;
...
FarestDist = 1000%1.5*median(depthMap(:)); % tried on university % nogood effect but keep it since usually it the rangelike this   % change to 1.5 for church
% ================================================
...
%             +set( ScaleP( setdiff( 1:NuSubSupSize, TriCol)) == 1);% only triangulated sup can shift (optional)
...
%           sol = solvesdp(F, norm( (NewPosiM*ParaPPCP-( ScaleP(col).*NewScalingTerm +ones(size(NewPosiM,1) ,1)) ) .* WeightsSelfTerm,1)...
          sol = solvesdp(F, norm( (NewPosiM*ParaPPCP-ScaleP(col) ) .* WeightsSelfTerm,1)...    
                    ...
                    + 0.1*Center*norm(((NewCoPM)*ParaPPCP + NewCoPMBound).*NewCoPEstDepth, 1)...%//Min
