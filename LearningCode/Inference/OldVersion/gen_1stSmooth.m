...
% *
% */
function [NonedgeY NonedgeX] = gen_1stSmooth(sup);
...
if nargin < 1
   sup = zeros(VertYNuDepth,HoriXNuDepth);
end
...
NonedgeX = zeros(VertYNuDepth, HoriXNuDepth);
 ...
 for i = NuIndex 
     mask = sup==i | sup==0;
     NonedgeY = NonedgeY | (conv2(double(mask),[1;1],'same'))==2;
     mask = sup==i;
     NonedgeX = NonedgeX | (conv2(double(mask),[1 1],'same'))==2;
 end    
NonedgeY = NonedgeY & sup~=0;
NonedgeY([end],:) = 0;
NonedgeX(:,[end]) = 0;
NonedgeY = NonedgeY(:);
NonedgeX = NonedgeX(:);
...
%Qy = spdiags([ones(NuPatch,1) -ones(NuPatch,1)],[0 1],NuPatch,NuPatch);
