...
% *
% */
function []=Depth_error_CMU(DepthDirectory, SkyExclude, detail,baseline)
...
% ======================= load Rajiv's .mat file
load([ScratchDataFolder '/data/distError' ImgFolder '.mat']);
MeanOffSet = mean(offset);
%CMUdepthNormalized=exp(offset)*CMUdepth;
% ============================================
...
for i= 1:NuPics
    ...
    depthMap = CMUdepth{i}*exp(MeanOffSet);
...
%       maskBigError = ((depthMap >1000) | (~useMask{i})) | (depthMap <=1); % 80+30
       maskBigError = ((depthMap >1000) ) | (depthMap <=1); % 80+30
%       maskBigError = (depthMap >Inf); % 80+30

       mask = ~(maskSky{i}|maskBigError);
       sum(mask(:))
%       any(isinf(log10(depthMap(:))))
%       any(isinf(log10(laserdepth(:))))
