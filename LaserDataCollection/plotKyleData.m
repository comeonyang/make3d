...
% *
% */
function plotKyleData(pseudoDepthMap,depthMapXRes,depthMapYRes, ...
    imgDirectory,imageFilename)

depthMapPlot = pseudoDepthMap(1:depthMapXRes,depthMapYRes:-1:1,3);
A = imread([imgDirectory imageFilename]);
A=imrotate(A,-90);

D_high = imresize(depthMapPlot, [size(A,1) size(A,2)], 'nearest');
B = rgb2ycbcr(A);
scaling_factor = floor(256/max(max(D_high)));
