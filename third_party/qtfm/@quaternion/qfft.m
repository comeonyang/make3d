function Y = qfft(X, A, L)
% QFFT Discrete quaternion Fourier transform.
%
% This function calculates the fast quaternion Fourier transform of
% (columns of) X. A specifies the transform axis (that is the direction
...
if isreal(X)
    ...
    C1 = fft(complex(scalar(X),      x(X)));
    C2 = fft(complex(     y(X), S .* z(X)));
    ...
    C1 = fft(complex(real(scalar(X)),      real(x(X))));
    C2 = fft(complex(imag(scalar(X)),      imag(x(X))));
    C3 = fft(complex(real(     y(X)), S .* real(z(X))));
    C4 = fft(complex(imag(     y(X)), S .* imag(z(X))));
