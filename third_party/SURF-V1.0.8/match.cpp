...
vector <int> jeffsMatches(const vector< Ipoint >& ipts1, const vector< Ipoint >& ipts2) {
  ...
  for (unsigned int i = 0; i < ipts1.size(); i++) {
    for (unsigned int j = i+1; j < ipts1.size(); j++) { 
  ...
  for (unsigned int i = 0; i < ipts2.size(); i++) {
    for (unsigned int j = i+1; j < ipts2.size(); j++) { 
  ...
  //image 1
  for (unsigned int i = 0; i < ipts1.size(); i++) {
    if (secondScore1[i] * 0.85 < bestScore1[i]) {
      bestIndex1[i] = -1;
    }
  }
  ...
  //image 2
  for (unsigned int i = 0; i < ipts2.size(); i++) {
    if (secondScore2[i] * 0.85 < bestScore2[i]) {
      bestIndex2[i] = -1;
    }
  }
...
int main(int argc, char **argv) {
	...
	vector< int > matches = jeffsMatches(ipts1, ipts2);
