...
/*	if ((y2 <= ConS1.ivec[7] && y2 >= -ConS1.ivec[7] && x2 <= ConS1.ivec[6] && x2 >= 0) || 
	    (y1 <= ConS2.ivec[7] && y1 >= -ConS2.ivec[7] && x1 <= ConS2.ivec[6] && x1 >= 0)){
		Sat = true;
	}
*/
...
vector <int> symMatches(const vector< Ipoint >& ipts1, const vector< Ipoint >& ipts2, const vector< Ipoint >& ConS1, const vector< Ipoint >& ConS2, const vector< Ipoint >& ConSRough1, const vector< Ipoint >& ConSRough2) {
  ...
  ******************************/
  ...
  for (unsigned int i = 0; i < ipts1.size(); i++) {
    if (secondScore1[i] * 0.7 < bestScore1[i] && bestScore1[i] < 0.4) {
      bestIndex1[i] = -1;
    }
  }
  ...
  for (unsigned int i = 0; i < ipts2.size(); i++) {
    if (secondScore2[i] * 0.7 < bestScore2[i] && bestScore2[i] < 0.4) {
      bestIndex2[i] = -1;
    }
  }
...
//	  cout << "test CONS" << ConS1[1].ivec[1] << endl;
		matches = symMatches(ipts1, ipts2, ConS1, ConS2, ConSRough1, ConSRough2);//Min
